...
9.  Server tiến hành chuỗi xác thực chống gian lận tự động:
    *   *Kiểm tra Lớp 1 (QR Token):* Khớp mã token động hiện hành; QR làm mới mỗi 10 giây và Server chấp nhận trong cửa sổ 15 giây.
    ...
    *   *Tín hiệu hỗ trợ (Wi-Fi Gateway):* Ghi nhận và kiểm tra chéo Public IP Gateway/SSID của mạng Wi-Fi trường; sai lệch được đánh dấu cảnh báo nhưng không tự động thay thế kết quả geofence.
10. Server xác thực tất cả thông tin hợp lệ, ghi nhận trạng thái `Present` hoặc `Late` vào bảng `AttendanceRecord` và xóa ảnh selfie Face ID tạm thời nếu có.
